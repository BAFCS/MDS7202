...
def home():
    data = {
        ...
        "modelo": "XGBoost, el cual es un algoritmo de aprendizaje supervisado basado en árboles de decisión que trabaja mediante una estrategia de crecimiento por niveles (level-wise), evaluando y dividiendo los nodos hoja de un nivel antes de pasar al siguiente, controlado por una profundidad máxima (max_depth) y técnicas de poda para evitar el sobreajuste.",
    ...
    )  # Para no tener problemas con caracteres especiales en la respuesta JSON, como los tildes
...
def predecir_potabilidad(data: MedicionAgua):
    features = np.array(
        ...
        1, -1
    )  # Convertimos la lista de características en un array de numpy y lo redimensionamos a una matriz de 1 fila y n columnas (donde n es el número de características)
